...
비디오 생성은 이미지 생성보다 훨씬 많은 GPU 리소스를 사용합니다. 크레딧은 **예상 GPU 실행 시간(초)**으로 산정합니다 (1 크레딧 = 1초 GPU 시간):

```
비디오 크레딧 = 모델별_초당_기본시간 × 프레임_수 × 해상도_배율

모델별 기본시간 (프레임당 GPU 초):
- AnimateDiff: 2.0초/프레임
- SVD:         4.0초/프레임
- Z-Image Video: 6.0초/프레임
- Hunyuan Video: 10.0초/프레임

해상도 배율 = (width × height) / (512 × 512)
- 512×512:   1.0
- 576×1024:  2.25
- 1280×720:  3.52

예시:
- 16프레임 512×512 AnimateDiff:    2.0 × 16 × 1.0  = 32 크레딧 (= ~32초 GPU)
- 25프레임 576×1024 SVD:           4.0 × 25 × 2.25 = 225 크레딧 (= ~3.75분 GPU)
- 48프레임 720p Z-Image Video:     6.0 × 48 × 3.52 = 1,014 크레딧 (= ~16.9분 GPU)
- 129프레임 720p Hunyuan Video:   10.0 × 129 × 3.52 = 4,541 크레딧 (= ~75.7분 GPU)
```
...
def _estimate_video_credits(num_frames: int, width: int, height: int, model: str) -> int:
    """비디오 크레딧 예측.
    1 credit = 1초 GPU 시간.
    비디오 생성 시간 = 모델별_초당_기본시간 × 프레임_수 × 해상도_배율
    """
    # 모델별 기준 시간 (1프레임당 초)
    MODEL_BASE_SECONDS = {
